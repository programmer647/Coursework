...
if (isset($_SESSION['orderid'])){
 ...
 //$stmt->bindParam(':orderid',$_SESSION['orderid']);
  ...
  while ($row=$stmt->fetch(PDO::FETCH_ASSOC))
  {
    ...
    echo($row['n'].', '.$row['name'].', ' .$row['Size1'].' '.$row['Size2'].', £'.$price.', '.$row['q'].', '.$row['new']);
    echo("</br>");
...
?>
<form action="transaction.php"><!--sets the page to redirect to when the button is pressed-->
...
<form action="remove.php"><!--sets the page to redirect to when the button is pressed-->
  <input type="submit" value="Remove Item"><!--Creates a button to complete the order by redirecting to the complete.php page-->
</form>
