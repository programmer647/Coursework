...
$stmt=$conn->prepare("SELECT count(*) from Tbluniform where UniformID=:uniformid");
$stmt->bindParam(':uniformid',$_POST['barcode']);
$stmt->execute();
...
if ($row['count(*)']!=0){
    ...
    $stmt=$conn->prepare("SELECT * FROM Tblbasket WHERE OrderID=:orderid and UniformID=:uniform and New=:new");//checks to see if there is already an item of the same type in the basket  
    //and if there is it takes the quantity of it so that this can be altered and then updated
