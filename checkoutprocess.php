...
if ($method=="home"){
    $stmt=$conn->prepare("UPDATE Tblorders SET AddressLine1=:address1, AddressLine2=:address2, Postcode=:postcode, Usercompleted=1 WHERE OrderID=:orderid");
...
elseif ($method=="collect"){
    echo('yes');
    $stmt=$conn->prepare("UPDATE Tblorders SET Email=:email, Usercompleted=1 WHERE OrderID=:orderid");
